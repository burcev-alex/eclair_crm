...
class CBPStudiobitBPCardAddButton
	extends \CBPActivity
{
	...
	public function Execute()
	{
		...
		if(intval($arDocumentId[1]) > 0 && strlen($this->buttonId)){
            ...
            $rsOption = $entity->getList([
                'filter' => $filter,
                'select' => ['ID', 'UF_BUTTONS']
            ]);

            if($arOption = $rsOption->fetch()){
                $buttons = unserialize($arOption['UF_BUTTONS']);
                if(!is_array($buttons))
                    $buttons = [];

                if(strlen($this->html))
                    $buttons[$this->buttonId] = $this->html;
                else
                    unset($buttons[$this->buttonId]);

                $arFields['UF_BUTTONS'] = serialize($buttons);
