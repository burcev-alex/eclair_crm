...
\App\Base\Entity\HighloadBlockTable::compileBaseClass('Command');

/**
 сущность для храненения и выполнения комманд по расписанию
 * 
 * примеры использования
 * $command = \App\Base\CommandManager::add();
 *
 * Повторяющаяся команда в модуле app.bigroup, выполнять каждый час в 20 и 40 минут
$command->module('app.bigroup')
 ->func('\App\BiGroup\Command\Example::run()')->minuts([20, 40])->regular(true)->save();
 *
 * Команда в модуле app.bigroup, выполнить один раз сейчас
$command->module('app.bigroup')
->func('\App\BiGroup\Command\Example::run()')->save();
 *
 * Повторяющаяся команда в модуле app.bigroup, выполнять 1 числа каждого месяца
$command->module('app.bigroup')->func('\App\BiGroup\Command\Example::run()')
 ->days(1)->regular(true)->save();
 *
 *  Команда в модуле app.bigroup, выполнить 01.01.2018 15:30:00
$command->module('app.bigroup')->func('\App\BiGroup\Command\Example::run()')
