...
class RestClient implements \Iterator, \ArrayAccess
{
	...
	{
		$strDebug = Main\Config\Option::get('app.base', 'debug', 'N');
	...
	public function execute($url, $method = 'GET', $parameters = [], $headers = [])
	{
		...
		if (strtoupper($method) == 'POST') {
			...
			$curlopt[CURLOPT_HTTPHEADER] = array_merge($curlopt[CURLOPT_HTTPHEADER], array(
		...
		$this->info = $client->info = (object)curl_getinfo($client->handle);
	...
	public function decode_response()
	{
		if (empty($this->decoded_response)) {
			...
			$this->decoded_response = call_user_func(
				...
				html_entity_decode($this->response), JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES
			);
		}
	...
	public function http_status_response()
	{
