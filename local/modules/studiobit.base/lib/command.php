...
class CommandManager
{
        ...
        while($arCommand = $rsCommands->fetch())
        {
            if(!static::isRun($arCommand)) {
                ...
                if(!isset($process[$arCommand['ID']])) {
                    \p2log(
                        \ConvertTimeStamp(false, 'FULL') . ': run command #' . $arCommand['ID'] . ' ' . $arCommand['UF_COMMAND'],
                ...
                \p2log(
                    \ConvertTimeStamp(false, 'FULL').': command #'.$arCommand['ID'].' working',
                    'command_'.\ConvertTimeStamp(false, 'SHORT')
                );
                //unset(static::$processes[$arCommand['ID']]);
            }
        }
    }
    ...
    public static function monitoringCommands()
    {
        ...
        return '\Studiobit\Base\CommandManager::monitoringCommands();';
    }
}