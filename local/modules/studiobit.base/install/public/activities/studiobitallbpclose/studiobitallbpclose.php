	...
	extends CBPActivity
{
	...
	public function Execute()
	{
        ...
        $arDocumentStates = CBPDocument::GetDocumentStates(
            $rootActivity->GetDocumentType(),
            $rootActivity->GetDocumentId()
        );

        foreach($arDocumentStates as $arDocument) {
            if ($arDocument['STATE_NAME'] !== 'Completed' && $arDocument['STATE_NAME'] !== 'Terminated' && $arDocument["ID"] > 0) {
                CBPDocument::TerminateWorkflow($arDocument["ID"], $rootActivity->GetDocumentId(), $arErrors = []);
            }
        }
