...
//переопределяем значения по умолчанию, общей карточкой
// $arResult['EDITOR']['ENTITY_CONFIG'] = \CUserOptions::GetOption(
    // 'crm.entity.editor',
    // $arResult['EDITOR']['CONFIG_ID'].'_common',
    // null,
    // 0
// );

if($entityID <= 0 && isset($arParams['SEARCH']))
{
    array_unshift(
        $tabs,
        array('id'=> 'main', 'name' => GetMessage("CRM_ENT_DETAIL_MAIN_TAB"), 'active' => false)
    );

    array_unshift(
        $tabs,
        array('id'=> 'search', 'name' => 'Поиск', 'active' => true, 'html' => $arParams['~SEARCH'])
    );
}
else
{
...
?><div id="<?=htmlspecialcharsbx($containerId)?>" class="crm-entity-wrap"><?
    ...
    elseif(isset($arParams['~PROGRESS_BAR_HTML'])){
        echo $arParams['~PROGRESS_BAR_HTML'];
    }
	...
	foreach($tabs as $tab)
	{
		...
		{
			?><div
                data-tab-id="<?=htmlspecialcharsbx($tabID)?>"
                class="crm-entity-section crm-entity-section-info<?if($tab['active'] !== true):?> crm-entity-section-tab-content-hide crm-entity-section-above-overlay<?endif;?>"
                <?if($tab['active'] !== true):?> style="display: none;"<?endif;?>
            ><?
				if(isset($tab['html']))
				{
					echo $tab['html'];
		...
		?><div data-tab-id="<?=htmlspecialcharsbx($tabID)?>"
               class="crm-entity-section crm-entity-section-info<?if($tab['active'] !== true):?> crm-entity-section-tab-content-hide crm-entity-section-above-overlay<?endif;?>"
            <?if($tab['active'] !== true):?> style="display: none;"<?endif;?>
        >
